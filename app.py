...
def returnloan():
    ...
    if request.method == "POST":
        ...
        loan_details = session.query(db.Loans).filter(
            db.Loans.customer_id == customer_id and db.Loans.book_id == book_id).first()
...
def loan():
    ...
    book_details = session.query(
        db.Books.id, db.Books.name, db.Books.type).all()
